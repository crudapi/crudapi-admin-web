/*
	Copyright (c) 2004-2011, The moor Foundation All Rights Reserved.
	Available via Academic Free License >= 2.1 OR the modified BSD license.
	see: http://moortoolkit.org/license for details
*/

/*
	This is an optimized version of moor, built for deployment and not for
	development. To get sources and documentation, please visit:

		http://moortoolkit.org
*/
...
/*=====
	moor.version = function(){
		// summary:
		//		Version number of the moor Toolkit
		// major: Integer
		...
		// revision: Number
		//		The SVN rev from which moor was pulled
		this.major = 0;
		this.minor = 0;
		this.patch = 0;
		this.flag = "";
...
=====*/
	...
	moor.mixin = function(/*Object*/obj, /*Object...*/props){
		...
		//		(now modified) obj.
		//	description:
		//		`moor.mixin` can mix multiple source objects into a
		//		destination object which is then returned. Unlike regular
		//		`for...in` iteration, `moor.mixin` is also smart about avoiding
		...
		//	|	var copy = moor.mixin({}, source);
		//	example:
		//		many class constructors often take an object which specifies
		//		values to be configured on the object. In this case, it is
		//		often simplest to call `moor.mixin` on the `this` object:
		//	|	moor.declare("acme.Base", null, {
		//	|		constructor: function(properties){
		//	|			// property configuration:
		//	|			moor.mixin(this, properties);
		//	|
	...
	moor.setObject = function(/*String*/name, /*Object*/value, /*Object?*/context){
		...
		//	|	moor.setObject("foo.bar.baz", value);
		//	example:
		//		without `moor.setObject`, we often see code like this:
		//	|	// ensure that intermediate objects are available
		//	|	if(!obj["parent"]){ obj.parent = {}; }
		//	|	if(!obj.parent["child"]){ obj.parent.child= {}; }
		//	|	// now we can safely set the property
		//	|	obj.parent.child.prop = "some value";
		//		wheras with `moor.setObject`, we can shorten that to:
		//	|	moor.setObject("parent.child.prop", "some value", obj);
	...
	moor.getObject = function(/*String*/name, /*Boolean?*/create, /*Object?*/context){
		...
		//	context:
		//		Optional. Object to use as root of path. Defaults to
		//		'moor.global'. Null may be passed.
	...
	moor.exists = function(/*String*/name, /*Object?*/obj){
		...
		//	|	moor.exists("foo.bar"); // true
		//	|	moor.exists("foo.bar.baz"); // false
		//	|
		//	|	// search from a particular scope
		//	|	moor.exists("bar", foo); // true
		//	|	moor.exists("bar.baz", foo); // false
	...
	/*=====
		moor.deprecated = function(behaviour, extra, removal){
			//	summary:
			...
			//	example:
			//	|	moor.deprecated("myApp.getTemp()", "use myApp.getLocaleTemp() instead", "1.0");
		}

		moor.experimental = function(moduleName, extra){
			...
			//	example:
			//	|	moor.experimental("moor.data.Result");
			//	example:
			//	|	moor.experimental("moor.weather.toKelvin()", "PENDING approval from NOAA");
		}
	=====*/
...
(function(){
	...
	moor.ready = moor.addOnLoad = function(/*Object*/obj, /*String|Function?*/functionName){
		// summary:
		//		Registers a function to be triggered after the DOM and moor.require() calls
		//		have finished loading.
		//
		// description:
		//		Registers a function to be triggered after the DOM has finished
		//		loading and `moor.require` modules have loaded. Widgets declared in markup
		...
		//	|	moor.addOnLoad(function(){ doStuff(); });
		//
		// example:
		...
		//	|	moor.addOnLoad(object, "functionName");
		//	|	moor.addOnLoad(object, function(){ doStuff(); });
	...
	moor.loadInit = function(/*Function*/init){
		//	summary:
		//		Executes a function that needs to be executed for the loader's moor.requireIf
		//		resolutions to work. This is needed mostly for the xdomain loader case where
		//		a function needs to be executed to set up the possible values for a moor.requireIf
		//		call.
		//	init:
		...
		//		on what is loaded, or what modules are available. Only the functionality in moor Base
		//		is allowed to be used. Avoid using this method. For a valid use case,
		//		see the source for moorx.gfx.
		init();
	}
	...
	moor._loadModule = moor.require = function(/*String*/moduleName, /*Boolean?*/omitModuleCheck){
		...
		//		 e.g. "moor.date.locale".  Module paths are de-referenced by moor's
		//		internal mapping of locations to names and are disambiguated by
		//		longest prefix. See `moor.registerModulePath()` for details on
		//		registering new modules.
		//
		//	omitModuleCheck: Boolean?
		//		if `true`, omitModuleCheck skips the step of ensuring that the
		//		loaded file actually defines the symbol it is referenced by.
		//		For example if it called as `moor.require("a.b.c")` and the
		//		file located at `a/b/c.js` does not define an object `a.b.c`,
		//		and exception will be throws whereas no exception is raised
		//		when called as `moor.require("a.b.c", true)`
		//
		//	description:
		// 		Modules are loaded via moor.require by using one of two loaders: the normal loader
		// 		and the xdomain loader. The xdomain loader is used when moor was built with a
		// 		custom build that specified loader=xdomain and the module lives on a modulePath
		// 		that is a whole URL, with protocol and a domain. The versions of moor that are on
		// 		the Google and AOL CDNs use the xdomain loader.
		//
		// 		If the module is loaded via the xdomain loader, it is an asynchronous load, since
		// 		the module is added via a dynamically created script tag. This
		// 		means that moor.require() can return before the module has loaded. However, this
		// 		should only happen in the case where you do moor.require calls in the top-level
		// 		HTML page, or if you purposely avoid the loader checking for moor.require
		// 		dependencies in your module by using a syntax like moor["require"] to load the module.
		//
		// 		Sometimes it is useful to not have the loader detect the moor.require calls in the
		// 		module so that you can dynamically load the modules as a result of an action on the
		// 		page, instead of right at module load time.
		//
		// 		Also, for script blocks in an HTML page, the loader does not pre-process them, so
		// 		it does not know to download the modules before the moor.require calls occur.
		//
		// 		So, in those two cases, when you want on-the-fly module loading or for script blocks
		// 		in the HTML page, special care must be taken if the moor.required code is loaded
		// 		asynchronously. To make sure you can execute code that depends on the moor.required
		// 		modules, be sure to add the code that depends on the modules in a moor.addOnLoad()
		// 		callback. moor.addOnLoad waits for all outstanding modules to finish loading before
		// 		executing.
		//
		...
		//
		// 		More on how moor.require
		//		`moor.require("A.B")` first checks to see if symbol A.B is
		//		defined. If it is, it is simply returned (nothing to do).
		//
		//		If it is not defined, it will look for `A/B.js` in the script root
		//		directory.
		//
		//		`moor.require` throws an exception if it cannot find a file
		//		to load, or if the symbol `A.B` is not defined after loading.
		//
		//		It returns the object `A.B`, but note the caveats above about on-the-fly loading and
		// 		HTML script blocks when the xdomain loader is loading a module.
		//
		//		`moor.require()` does nothing about importing symbols into
		//		the current namespace.  It is presumed that the caller will
		//		take care of that.
		//
		// 	example:
		// 		To use moor.require in conjunction with moor.ready:
		//
		//		|	moor.require("foo");
		//		|	moor.require("bar");
		//	   	|	moor.addOnLoad(function(){
		...
		//
		//		|	with (moor.require("A.B")) {
		//		|		...
		//		|	}
		//
		//		And to import just the leaf symbol to a local variable:
		//
		//		|	var B = moor.require("A.B");
		//	   	|	...
		//
		//	returns:
	...
	moor.provide = function(/*String*/ resourceName){
		//	summary:
		//		Register a resource with the package system. Works in conjunction with `moor.require`
		//
		//	description:
		//		Each javascript source file is called a resource.  When a
		//		resource is loaded by the browser, `moor.provide()` registers
		//		that it has been loaded.
		//
		//		Each javascript source file must have at least one
		//		`moor.provide()` call at the top of the file, corresponding to
		//		the file name.  For example, `js/moor/foo.js` must have
		//		`moor.provide("moor.foo");` before any calls to
		//		`moor.require()` are made.
		//
		//		For backwards compatibility reasons, in addition to registering
		//		the resource, `moor.provide()` also ensures that the javascript
		//		object for the module exists.  For example,
		//		`moor.provide("moorx.data.FlickrStore")`, in addition to
		//		registering that `FlickrStore.js` is a resource for the
		//		`moorx.data` module, will ensure that the `moorx.data`
		//		javascript object exists, so that calls like
		//		`moor.data.foo = function(){ ... }` don't fail.
		//
		//		In the case of a build where multiple javascript source files
		//		are combined into one bigger file (similar to a .lib or .jar
		//		file), that file may contain multiple moor.provide() calls, to
		...
		//
		// example:
		//	Safely create a `my` object, and make moor.require("my.CustomModule") work
		//	|	moor.provide("my.CustomModule");
	...
	moor.platformRequire = function(/*Object*/modMap){
		...
		//		moor is currently operating in
		//	description:
		//		This method takes a "map" of arrays which one can use to
		//		optionally load moor modules. The map is indexed by the
		//		possible moor.name_ values, with two additional values:
		//		"default" and "common". The items in the "default" array will
		//		be loaded if none of the other items have been choosen based on
		//		moor.name_, set by your host environment. The items in the
		//		"common" array will *always* be loaded, regardless of which
		//		list is chosen.
		//	example:
		//		|	moor.platformRequire({
		//		|		browser: [
		//		|			"foo.sample", // simple module
		//		|			"foo.test",
		//		|			["foo.bar.baz", true] // skip object check in _loadModule (moor.require)
		//		|		],
	...
	moor.registerModulePath = function(/*String*/module, /*String*/prefix){
		...
		//		relative to moor root. For example, module acme is mapped to
		//		../acme.  If you want to use a different module name, use
		//		moor.registerModulePath.
		//	example:
		//		If your moor.js is located at this location in the web root:
		//	|	/myapp/js/moor/moor/moor.js
		//		and your modules are located at:
		//	|	/myapp/js/foo/bar.js
		//	|	/myapp/js/foo/baz.js
		//	|	/myapp/js/foo/thud/xyzzy.js
		//		Your application can tell moor to locate the "foo" namespace by calling:
		//	|	moor.registerModulePath("foo", "../../foo");
		//		At which point you can then use moor.require() to load the
		//		modules (assuming they provide() the same things which are
		//		required). The full code might be:
		//	|	<script type="text/javascript"
		//	|		src="/myapp/js/moor/moor/moor.js"></script>
		//	|	<script type="text/javascript">
		//	|		moor.registerModulePath("foo", "../../foo");
		//	|		moor.require("foo.bar");
		//	|		moor.require("foo.baz");
		//	|		moor.require("foo.thud.xyzzy");
		//	|	</script>
	...
	moor.requireLocalization = function(/*String*/moduleName, /*String*/bundleName, /*String?*/locale, /*String?*/availableFlatLocales){
		...
		//		same style as moor.require.  Contents of the resource bundle are
		//		typically strings, but may be any name/value pair, represented in
		//		JSON format.  See also `moor.i18n.getLocalization`.
		//
		// description:
		...
		//		locale as defined by moor.locale
		//
	...
	moor._Url = function(/*moor._Url|String...*/){
		...
		//      	new moor._Url(document.baseURI, url)
	...
	moor.moduleUrl = function(/*String*/module, /*moor._Url||String*/url){
		//	summary:
		//		Returns a `moor._Url` object relative to a module.
		//	example:
		//	|	var pngPath = moor.moduleUrl("acme","images/small.png");
		...
		//	|	moor.body().appendChild(img);
		//	example:
		//		you may de-reference as far as you like down the package
		//		hierarchy.  This is sometimes handy to avoid lenghty relative
		//		urls or for building portable sub-packages. In this example,
		//		the `acme.widget` and `acme.util` directories may be located
		//		under different roots (see `moor.registerModulePath`) but the
		//		the modules which reference them can be unaware of their
		//		relative locations on the filesystem:
		//	|	// somewhere in a configuration block
		//	|	moor.registerModulePath("acme.widget", "../../acme/widget");
		//	|	moor.registerModulePath("acme.util", "../../util");
		//	|
		//	|	// ...
		//	|
		//	|	// code in a module using acme resources
		//	|	var tmpltPath = moor.moduleUrl("acme.widget","templates/template.html");
		//	|	var dataPath = moor.moduleUrl("acme.util","resources/data.json");
...
moor.isSafari = {
	...
	//	example:
	//		Detect iPhone:
	//	|	if(moor.isSafari && navigator.userAgent.indexOf("iPhone") != -1){
...
if(typeof window != 'undefined'){
	...
	(function(){
		...
		d.addOnWindowUnload = function(/*Object?|Function?*/obj, /*String|Function?*/functionName){
			...
			//		fires.
			//	description:
			//		The first time that addOnWindowUnload is called moor
			...
			//		moor.addOnUnload() if you need to modify the DOM or do
			//		heavy JavaScript work since it fires at the eqivalent of
			//		the page's "onbeforeunload" event.
			// example:
			//	|	moor.addOnWindowUnload(functionPointer)
			//	|	moor.addOnWindowUnload(object, "functionName");
			//	|	moor.addOnWindowUnload(object, function(){ /* ... */});
		...
		d.addOnUnload = function(/*Object?|Function?*/obj, /*String|Function?*/functionName){
			...
			//	description:
			//		The first time that addOnUnload is called moor will
			...
			//		operations in a moor.addOnUnload callback.
			//
			//		Further note that calling moor.addOnUnload will prevent
			//		browsers from using a "fast back" cache to make page
			//		loading via back button instantaneous.
			// example:
			//	|	moor.addOnUnload(functionPointer)
			//	|	moor.addOnUnload(object, "functionName")
			//	|	moor.addOnUnload(object, function(){ /* ... */});
...
(function(){
	...
	moor.mixin(moor, {
		...
		forEach: function(/*Array|String*/arr, /*Function|String*/callback, /*Object?*/thisObject){
			...
			//		If you want to break out of the loop, consider using moor.every() or moor.some().
			...
			//	|	);
			//	|
			//	|	// alternately, we can accomplish the same thing with moor.hitch()
			//	|	moor.forEach(
			//	|		[ "thinger", "blah", "howdy", 10 ],
			//	|		moor.hitch(obj, "callback")
			//	|	);
		...
		every: function(/*Array|String*/arr, /*Function|String*/callback, /*Object?*/thisObject){
			...
			// example:
			//	|	// returns false
			//	|	moor.every([1, 2, 3, 4], function(item){ return item>1; });
			// example:
			//	|	// returns true
			//	|	moor.every([1, 2, 3, 4], function(item){ return item>0; });
		...
		some: function(/*Array|String*/arr, /*Function|String*/callback, /*Object?*/thisObject){
			...
			// example:
			//	|	// is true
			//	|	moor.some([1, 2, 3, 4], function(item){ return item>1; });
			// example:
			//	|	// is false
			//	|	moor.some([1, 2, 3, 4], function(item){ return item<1; });
		...
		map: function(/*Array|String*/arr, /*Function|String*/callback, /*Function?*/thisObject){
			...
			// example:
			//	|	// returns [2, 3, 4, 5]
			//	|	moor.map([1, 2, 3, 4], function(item){ return item+1 });
		...
		filter: function(/*Array*/arr, /*Function|String*/callback, /*Object?*/thisObject){
			...
			// example:
			//	|	// returns [2, 3, 4]
			//	|	moor.filter([1, 2, 3, 4], function(item){ return item>1; });
...
(function(){
	...
	/*=====
	moor.declare = function(className, superclass, props){
		//	summary:
		...
		//		moor.declare. In complex cases all base classes are going to be
		...
		//		moor.declare) as bases. They will be called as usual during the
		...
		//
		//	example:
		//	|	moor.declare("my.classes.bar", my.classes.foo, {
		...
		//
		//	example:
		//	|	var MyBase = moor.declare(null, {
		//	|		// constructor, properties, and methods go here
		//	|		// ...
		//	|	});
		//	|	var MyClass1 = moor.declare(MyBase, {
		//	|		// constructor, properties, and methods go here
		//	|		// ...
		//	|	});
		//	|	var MyClass2 = moor.declare(MyBase, {
		//	|		// constructor, properties, and methods go here
		//	|		// ...
		//	|	});
		//	|	var MyDiamond = moor.declare([MyClass1, MyClass2], {
		//	|		// constructor, properties, and methods go here
		//	|		// ...
		//	|	});
		//
		...
		//
		//	example:
		//	|	var A = moor.declare(null, {
		//	|		"-chains-": {
		//	|			destroy: "before"
		//	|		}
		//	|	});
		//	|	var B = moor.declare(A, {
		//	|		constructor: function(){
	...
	/*=====
	moor.safeMixin = function(target, source){
		//	summary:
		//		Mix in properties skipping a constructor and decorating functions
		//		like it is done by moor.declare.
		//	target: Object
		//		Target object to accept new properties.
		//	source: Object
		//		Source object for new properties.
		//	description:
		//		This function is used to mix in properties like moor._mixin does,
		//		but it skips a constructor property and decorates functions like
		//		moor.declare does.
		//
		//		It is meant to be used with classes and objects produced with
		//		moor.declare. Functions mixed in with moor.safeMixin can use
		//		this.inherited() like normal methods.
		//
		//		This function is used to implement extend() method of a constructor
		//		produced with moor.declare().
		//
		//	example:
		//	|	var A = moor.declare(null, {
		//	|		m1: function(){
	...
	Object.inherited = function(name, args, newArgs){
		...
		//		moor.declare, and it cannot be determined.
		//	args: Arguments
		...
		//		moor.declare to call a super method (next in the chain). It is
	...
	Object.isInstanceOf = function(cls){
		...
		//		moor.declare to determine of they support a certain interface or
		...
		//	|		// ...
		//	|	});
		//	|	var B = moor.declare(null, {
		//	|		// constructor, properties, and methods go here
		//	|		// ...
		//	|	});
		//	|	var C = moor.declare([A, B], {
		//	|		// constructor, properties, and methods go here
		//	|		// ...
		//	|	});
		//	|	var D = moor.declare(A, {
		//	|		// constructor, properties, and methods go here
		//	|		// ...
		//	|	});
		//	|
	...
	Object.extend = function(source){
		...
		//
		//		This method is similar to moor.extend function, but it is specific
		//		to constructors produced by moor.declare. It is implemented
		//		using moor.safeMixin, and it skips a constructor property,
...
moor.connect = function(/*Object|null*/ obj,
						...
						/*Boolean?*/ dontFix){
	// summary:
	//		`moor.connect` is the core event handling and delegation method in
	//		moor. It allows one function to "listen in" on the execution of
	...
	//		function.
	//
	//		Since `moor.connect` allows the source of events to be either a
	...
	//		`obj` is null, `moor.global` is assumed, meaning that connections
	//		to global methods are supported but also that you may inadvertently
	//		connect to a global by passing an incorrect object name or invalid
	//		reference.
	//
	//		`moor.connect` generally is forgiving. If you pass the name of a
	...
	//		remove this connection with `moor.disconnect`.
	//
	// obj:
	//		The source object for the event function.
	//		Defaults to `moor.global` if null.
	...
	//	|	moor.connect(obj, "onchange", ui, "update");
	//	|	moor.connect(obj, "onchange", ui, ui.update); // same
	//
	// example:
	//		Using return value for disconnect:
	//	|	var link = moor.connect(obj, "onchange", ui, "update");
	//	|	...
	//	|	moor.disconnect(link);
	//
	// example:
	//		When onglobalevent executes, watcher.handler is invoked:
	//	|	moor.connect(null, "onglobalevent", watcher, "handler");
	//
	// example:
	//		When ob.onCustomEvent executes, customEventHandler is invoked:
	//	|	moor.connect(ob, "onCustomEvent", null, "customEventHandler");
	//	|	moor.connect(ob, "onCustomEvent", "customEventHandler"); // same
	//
	// example:
	//		When ob.onCustomEvent executes, customEventHandler is invoked
	...
	//		with the same scope (this):
	//	|	moor.connect(null, "globalEvent", null, globalHandler);
	//	|	moor.connect("globalEvent", globalHandler); // same
...
(function(){
	...
	// description:
	//		The moor.Deferred API is based on the concept of promises that provide a
	...
	//
	//  	The moor.Deferred is a type of promise that provides methods for fulfilling the
	// 		promise with a successful result or an error. The most important method for
	// 		working with moor's promises is the then() method, which follows the
	// 		CommonJS proposed promise API. An example of using a moor promise:
	//
	...
	//
	//		The moor.Deferred instances also provide the following functions for backwards compatibility:
	//
	//			* addCallback(handler)
	//			* addErrback(handler)
	//			* callback(result)
	...
	// example:
	//	|	var deferred = new moor.Deferred();
...
moor.toJson = function(/*Object*/ it, /*Boolean?*/ prettyPrint, /*String?*/ _indentStr){
	...
	//		The variable `moor.toJsonIndentStr` is used as the indent string --
	//		to use something other than the default (tab), change that variable
	//		before calling moor.toJson().
	//	_indentStr:
	//		private variable for recursive calls when pretty printing, do not use.
	//	example:
	//		simple serialization of a trivial object
	//		|	var jsonStr = moor.toJson({ howdy: "stranger!", isStrange: true });
	//		|	doh.is('{"howdy":"stranger!","isStrange":true}', jsonStr);
	//	example:
	//		a custom serializer for an objects of a particular class:
	//		|	moor.declare("Furby", null, {
	//		|		furbies: "are strange",
	//		|		furbyCount: 10,
	//		|		__json__: function(){
	//		|		},
...
/*=====
	moor.mouseButtons = {
		...
		isButton: function(e, button){
			...
			//		Event object to examine
			// button: Number
			//		The button value (example: moor.mouseButton.LEFT)
...
moor.byId = function(id, doc){
	...
	//		moor.doc.  Can be used to retrieve
...
=====*/
	moor._destroyElement = moor.destroy = function(/*String|DomNode*/node){
		...
		//	|	moor.query(".someNode").forEach(moor.destroy);
	...
	moor.place = function(node, refNode, position){
		...
		//
		//		.place() is also a method of `moor.NodeList`, allowing `moor.query` node lookups.
		//
		// example:
		//		Place a node by string id as the last child of another node by string id:
		//	|	moor.place("someNode", "anotherNode");
		//
		// example:
		//		Place a node by string id before another node by string id
		//	|	moor.place("someNode", "anotherNode", "before");
		//
		// example:
		//		Create a Node, and place it in the body element (last child):
		//	|	moor.place("<div></div>", moor.body());
		//
		// example:
		//		Put a new LI as the first child of a list by id:
		//	|	moor.place("<li></li>", "someUl", "first");
...
/*=====
	moor.getComputedStyle = function(node){
		//	summary:
		...
		//
		//		Use the moor.style() method for more consistent (pixelized)
		//		return values.
		//
		//	node: DOMNode
		...
		//	example:
		//	|	moor.getComputedStyle(moor.byId('foo')).borderWidth;
		//
		//	example:
		//	Reusing the returned object, avoiding multiple lookups:
		//	|	var cs = moor.getComputedStyle(moor.byId("someNode"));
...
=====*/
	...
	moor.style = function(	/*DomNode|String*/ node,
		...
		//		If you want to get node dimensions, use `moor.marginBox()`,
		//		`moor.contentBox()` or `moor.position()`.
		//	node:
		...
		//	|	moor.style("thinger", "opacity"); // 1 by default
		//
		//	example:
		//		Passing a node, a style property, and a value changes the
		//		current display of the node and returns the new computed value
		//	|	moor.style("thinger", "opacity", 0.5); // == 0.5
		//
		//	example:
		...
		//
		//	example:
		//		moor.NodeList implements .style() using the same syntax, omitting the "node" parameter, calling
		//		moor.style() on every element of the list. See: `moor.query()` and `moor.NodeList()`
		//	|	moor.query(".someClassName").style("visibility","hidden");
		//	|	// or
		//	|	moor.query("#baz > div").style({
		//	|		opacity:0.75,
		//	|		fontSize:"13pt"
		//	|	});
	...
	moor.marginBox = function(/*DomNode|String*/node, /*Object?*/box){
		...
		//	box:
		//		If passed, denotes that moor.marginBox() should
		//		update/set the margin box for node. Box is an object in the
		//		above format. All properties are optional if passed.
		//	example:
		//	Retrieve the marginbox of a passed node
		//	|	var box = moor.marginBox("someNodeId");
		//	|	console.dir(box);
		//
		//	example:
		//	Set a node's marginbox to the size of another node
		//	|	var box = moor.marginBox("someNodeId");
		//	|	moor.marginBox("someOtherNode", box);
	...
	moor.contentBox = function(/*DomNode|String*/node, /*Object?*/box){
		...
		//		id or reference to DOM Node to get/set box for
		//	box:
		//		If passed, denotes that moor.contentBox() should
	...
	moor.attr = function(/*DomNode|String*/node, /*String|Object*/name, /*String?*/value){
		...
		//		using `moor.connect()`, meaning that event handler properties
		//		will be normalized and that some caveats with regards to
		//		non-standard behaviors for onsubmit apply. Namely that you
		//		should cancel form submission using `moor.stopEvent()` on the
		...
		//	|	moor.attr(moor.byId("nodeId"), "foo");
		//	|	// or we can just pass the id:
		//	|	moor.attr("nodeId", "foo");
		//
		//	example:
		//	|	// use attr() to set the tab index
		//	|	moor.attr("nodeId", "tabIndex", 3);
		//	|
		//
		//	example:
		...
		//	|			// moor.stopEvent() to ensure that the submission
		//	|			// doesn't proceed.
		//	|			moor.stopEvent(e);
		//	|
		//	|			// submit the form with Ajax
		//	|			moor.xhrPost({ form: "formId" });
		//	|		}
		//	|	});
		//
		//	example:
		...
		//	|	moor.attr("someNode", "style", obj);
		//	|
		//	|	// though shorter to use `moor.style()` in this case:
		//	|	moor.style("someNode", obj);
	...
	moor.create = function(tag, attrs, refNode, pos){
		...
		//|
		//		Attributes are set by passing the optional object through `moor.attr`.
		//		See `moor.attr` for noted caveats and nuances, and API if applicable.
		//|
		//		Placement is done via `moor.place`, assuming the new node to be the action
		...
		//		See: `moor.attr` for a description of available attributes.
		//
		// refNode: String?|DomNode?
		//		Optional reference node. Used by `moor.place` to place the newly created
		//		node somewhere in the dom relative to refNode. Can be a DomNode reference
		//		or String ID of a node.
		//
		// pos: String?
		//		Optional positional reference. Defaults to "last" by way of `moor.place`,
		...
		// example:
		//	Create a DIV:
		//	|	var n = moor.create("div");
		//
		// example:
		//	Create a DIV with content:
		//	|	var n = moor.create("div", { innerHTML:"<p>hi</p>" });
		//
		// example:
		//	Place a new DIV in the BODY, with no attributes set
		//	|	var n = moor.create("div", null, moor.body());
		//
		// example:
		//	Create an UL, and populate it with LI's. Place the list as the first-child of a
		//	node with id="someId":
		//	|	var ul = moor.create("ul", null, "someId", "first");
		//	|	var items = ["one", "two", "three", "four"];
		//	|	moor.forEach(items, function(data){
		//	|		moor.create("li", { innerHTML: data }, ul);
		//	|	});
		//
		// example:
		//	Create an anchor, with an href. Place in BODY:
		//	|	moor.create("a", { href:"foo.html", title:"Goto FOO!" }, moor.body());
		//
		// example:
		//	Create a `moor.NodeList()` from a new element (for syntatic sugar):
		//	|	moor.query(moor.create('div'))
		//	|		.addClass("newDiv")
	...
	moor.addClass = function(/*DomNode|String*/node, /*String|Array*/classStr){
		...
		//	|	moor.addClass("someNode", "anewClass");
		//
		// example:
		//	Add two classes at once:
		//	|	moor.addClass("someNode", "firstClass secondClass");
		//
		// example:
		...
		//
		// example:
		//	Available in `moor.NodeList` for multiple additions
		//	|	moor.query("ul > li").addClass("firstLevel");
	...
	moor.removeClass = function(/*DomNode|String*/node, /*String|Array?*/classStr){
		// summary:
		//		Removes the specified classes from node. No `moor.hasClass`
		//		check is required.
		//
		// node:
		...
		//	|	moor.removeClass("someNode", "firstClass");
		//
		// example:
		//	Remove two classes from some node:
		//	|	moor.removeClass("someNode", "firstClass secondClass");
		//
		// example:
		...
		//	|	moor.removeClass("someNode");
		//
		// example:
		//	Available in `moor.NodeList()` for multiple removal
		//	|	moor.query(".foo").removeClass("foo");
	...
	moor.replaceClass = function(/*DomNode|String*/node, /*String|Array*/addClassStr, /*String|Array?*/removeClassStr){
		...
		//		Operates more quickly than calling moor.removeClass and moor.addClass
		// node:
		// 		String ID or DomNode reference to remove the class from.
		// addClassStr:
		//		A String class name to add, or several space-separated class names,
		...
		//
		// example:
		//	|	moor.replaceClass("someNode", "add1 add2", "remove1 remove2");
		//
		// example:
		//	Replace all classes with addMe
		//	|	moor.replaceClass("someNode", "addMe");
		//
		// example:
		//	Available in `moor.NodeList()` for multiple toggles
		//	|	moor.query(".findMe").replaceClass("addMe", "removeMe");
...
(function(){
	...
	var tnl = function(/*Array*/ a, /*moor.NodeList?*/ parent, /*Function?*/ NodeListCtor){
		// summary:
		// 		decorate an array to make it look like a `moor.NodeList`.
		// a:
		...
		// 		NodeList calls to use a different object than moor.NodeList.
		if(!a.sort){
	...
	moor.NodeList = function(){
		//	summary:
		//		moor.NodeList is an of Array subclass which adds syntactic
		//		sugar for chaining, common iteration operations, animation, and
		//		node manipulation. NodeLists are most often returned as the
		//		result of moor.query() calls.
		//	description:
		//		moor.NodeList instances provide many utilities that reflect
		//		core moor APIs for Array iteration and manipulation, DOM
		//		manipulation, and event handling. Instead of needing to dig up
		//		functions in the moor.* namespace, NodeLists generally make the
		//		full power of moor available for DOM manipulation tasks in a
		//		simple, chainable way.
		//	example:
		//		create a node list from a node
		//		|	new moor.NodeList(moor.byId("foo"));
		//	example:
		//		get a NodeList from a CSS query and iterate on it
		//		|	var l = moor.query(".thinger");
		//		|	l.forEach(function(node, index, nodeList){
		//		|		console.log(index, node.innerHTML);
		//		|	});
		//	example:
		//		use native and moor-provided array methods to manipulate a
		//		NodeList without needing to use moor.* functions explicitly:
		//		|	var l = moor.query(".thinger");
		//		|	// since NodeLists are real arrays, they have a length
		//		|	// property that is both readable and writable and
		//		|	// push/pop/shift/unshift methods
		//		|	console.log(l.length);
		//		|	l.push(moor.create("span"));
		//		|
		//		|	// moor's normalized array methods work too:
		//		|	console.log( l.indexOf(moor.byId("foo")) );
		...
		//		|	// style() as a getter...
		//		|	var borders = moor.query(".thinger").style("border");
		//		|	// ...and as a setter:
		//		|	moor.query(".thinger").style("border", "1px solid black");
		//		|	// class manipulation
		//		|	moor.query("li:nth-child(even)").addClass("even");
		//		|	// even getting the coordinates of all the items
		//		|	var coords = moor.query(".thinger").coords();
		//	example:
		//		DOM manipulation functions from the moor.* namespace area also
		//		available:
		//		|	// remove all of the elements in the list from their
		//		|	// parents (akin to "deleting" them from the document)
		//		|	moor.query(".thinger").orphan();
		//		|	// place all elements in the list at the front of #foo
		//		|	moor.query(".thinger").place("foo", "first");
		//	example:
		//		Event handling couldn't be easier. `moor.connect` is mapped in,
		//		and shortcut handlers are provided for most DOM events:
		//		|	// like moor.connect(), but with implicit scope
		//		|	moor.query("li").connect("onclick", console, "log");
		//		|
		//		|	// many common event handlers are already available directly:
		//		|	moor.query("li").onclick(console, "log");
		//		|	var toggleHovered = moor.hitch(moor, "toggleClass", "hovered");
		//		|	moor.query("p")
	...
	moor.extend(moor.NodeList, {
		_normalize: function(/*String||Element||Object||NodeList*/content, /*DOMNode?*/refNode){
			...
			// 		moor.string.substitute (if moor.string.substitute has been moor.required elsewhere),
			// 		or if templateFunc is a function on the content, that function will be used to
			// 		transform the template into a final string to be used for for passing to moor._toDom.
			// 		If content.parse is true, then it is remembered for later, for when the content
			// 		nodes are inserted into the DOM. At that point, the nodes will be parsed for widgets
			// 		(if moor.parser has been moor.required elsewhere).
		...
		_stash: function(parent){
			...
			//
			// example:
			// How to make a `moor.NodeList` method that only returns the third node in
			// the moor.NodeList but allows access to the original NodeList by using this._stash:
			//	|	moor.extend(moor.NodeList, {
			//	|		third: function(){
			//  |			var newNodeList = moor.NodeList(this[2]);
			...
			// summary:
			// 		Ends use of the current `moor.NodeList` by returning the previous moor.NodeList
			// 		that generated the current moor.NodeList.
			// description:
			// 		Returns the `moor.NodeList` that generated the current `moor.NodeList`. If there
			// 		is no parent moor.NodeList, an empty moor.NodeList is returned.
			// example:
			//	|	moor.query("a")
			//	|		.filter(".disabled")
		...
		//		http://trac.moortoolkit.org/ticket/3244
		...
		slice: function(begin, end){
			...
			//		with the caveat that it returns a moor.NodeList and not a
		...
		indexOf: function(value, fromIndex){
			//	summary:
			//		see moor.indexOf(). The primary difference is that the acted-on
		...
		lastIndexOf: function(value, fromIndex){
			// summary:
			//		see moor.lastIndexOf(). The primary difference is that the
		...
		every: function(callback, thisObject){
			//	summary:
			//		see `moor.every()` and the (Array.every
			//		docs)[http://developer.mozilla.org/en/docs/Core_JavaScript_1.5_Reference:Global_Objects:Array:every].
			//		Takes the same structure of arguments and returns as
			//		moor.every() with the caveat that the passed array is
		...
		some: function(callback, thisObject){
			//	summary:
			//		Takes the same structure of arguments and returns as
			//		`moor.some()` with the caveat that the passed array is
			//		implicitly this NodeList.  See `moor.some()` and Mozilla's
			//		(Array.some
		...
		concat: function(item){
			...
			// native NodeList and moor.NodeList in this property to recognize
			// the node list.
		...
		coords: function(){
			...
			//		an Array (*not* a NodeList). Acts like `moor.coords`, though assumes
		...
		position: function(){
			...
			//		as an Array (*not* a NodeList). Acts like `moor.position`, though
		...
		attr: function(property, value){
			...
			//	|	moor.query(".focusable").attr("tabIndex", -1);
			//	example:
			//		Disable a group of buttons:
			//	|	moor.query("button.group").attr("disabled", true);
			//	example:
			//		innerHTML can be assigned or retrieved as well:
			//	|	// get the innerHTML (as an array) for each list item
			//	|	var ih = moor.query("li.replaceable").attr("innerHTML");
			return; // moor.NodeList
			return; // Array
		},
		...
		connect: function(methodName, objOrFunc, funcName){
			//	summary:
			//		attach event handlers to every item of the NodeList. Uses moor.connect()
			...
			//		|	moor.query("div:nth-child(odd)").connect("onclick", function(e){
			//		|		console.log("clicked!");
			//		|	});
			// example:
			//		attach foo.bar() to every odd div's onmouseover
			//		|	moor.query("div:nth-child(odd)").connect("onmouseover", foo, "bar");
		},
		...
		empty: function(){
			...
			//		the list.
			return this.forEach("item.innerHTML='';"); // moor.NodeList
		...
		*/
		...
		place: function(/*String||Node*/ queryOrNode, /*String*/ position){
			...
			//		by queryOrNode. Returns the original NodeList. See: `moor.place`
			//	queryOrNode:
		...
		adopt: function(/*String||Array||DomNode*/ queryOrListOrNode, /*String?*/ position){
			...
			//		Returns a moor.NodeList of the adopted elements.
			//	queryOrListOrNode:
		...
		query: function(/*String*/ queryStr){
			...
			//	|	var l = new moor.NodeList(moor.byId("foo"), moor.byId("bar"));
		...
		*/
		...
		addContent: function(/*String||DomNode||Object||moor.NodeList*/ content, /*String||Integer?*/ position){
			...
			// 		to insert. If moor.string has already been moor.required, then moor.string.substitute
			// 		will be used on the "template" to generate the final HTML string. Other allowed
			// 		properties on the object are: "parse" if the HTML
			// 		string should be parsed for widgets (moor.require("moor.parser") to get that
			// 		option to work), and "templateFunc" if a template function besides moor.string.substitute
			...
			//	|	moor.query("h3 > p").addContent("hey there!");
			//	example:
			//		add something to the front of each element that has a
			//		"thinger" property:
			//	|	moor.query("[thinger]").addContent("...", "first");
			//	example:
			//		adds a header before each element of the list
			//	|	moor.query(".note").addContent("<h4>NOTE:</h4>", "before");
			//	example:
			//		add a clone of a DOM node to the end of every element in
			//		the list, removing it from its existing parent.
			//	|	moor.query(".note").addContent(moor.byId("foo"));
			//  example:
			//  	Append nodes from a templatized string.
			// 		moor.require("moor.string");
			// 		moor.query(".note").addContent({
			...
			//  	moor.require("moor.string");
			//  	moor.require("moor.parser");
			//  	var notes = moor.query(".note").addContent({
			//  		template: '<button moorType="dijit.form.Button">${text}</button>',
			//  		parse: true,
			//  		text: "Send"
			//  	});
		...
		at: function(/*===== index =====*/){
			...
			//	|	moor.query("a").at(0, 1, 2).forEach(fn);
			//
			//	example:
			//	Retrieve the first and last elements of a unordered list:
			//	|	moor.query("ul > li").at(0, -1).forEach(cb);
			//
			//	example:
			//	Do something for the first element only, but end() out back to
			//	the original list and continue chaining:
			//	|	moor.query("a").at(0).onclick(fn).end().forEach(function(n){
			...
			nlp[_oe] = function(a, b){
				...
				return (a ? this.connect(_oe, a, b) :
							this.forEach(function(n){
								...
								// moor._event_listener._synthesizeEvent({}, { target: n, faux: true, type: evt });
...
/*
	moor.query() architectural overview:

		moor.query is a relatively full-featured CSS3 query library. It is
...
var defineQuery= function(d){
	...
	var getQueryParts = function(query){
		...
		//		moor.query implements an AST-style query representation. This
	...
	*/
	...
	d.query = function(/*String*/ query, /*String|DOMNode?*/ root){
		...
		//		the search by. Returns an instance of moor.NodeList.
		//	description:
		//		moor.query() is the swiss army knife of DOM node manipulation in
		//		moor. Much like Prototype's "$$" (bling-bling) function or JQuery's
		//		"$" function, moor.query provides robust, high-performance
		...
		//		--------------------
		//
		//		moor.query() supports a rich set of CSS3 selectors, including:
		//
		...
		//		`moor.query()`, including compound selectors ("," delimited).
		//		Very complex and useful searches can be constructed with this
		//		palette of selectors and when combined with functions for
		//		manipulation presented by moor.NodeList, many types of DOM
		//		manipulation operations become very straightforward.
		//
		//		Unsupported Selectors:
		//		----------------------
		//
		//		While moor.query handles many CSS3 selectors, some fall outside of
		...
		//
		//		moor.query and XML Documents:
		//		-----------------------------
		//
		//		`moor.query` (as of moor 1.2) supports searching XML documents
		//		in a case-sensitive manner. If an HTML document is served with
		//		a doctype that forces case-sensitivity (e.g., XHTML 1.1
		//		Strict), moor.query() will detect this and "do the right
		...
		//		`moor.query` will return a new `moor.NodeList` instance
		//		constructed from that parameter alone and all further
		//		processing will stop. This means that if you have a reference
		//		to a node or NodeList, you can quickly construct a new NodeList
		//		from the original by calling `moor.query(node)` or
		//		`moor.query(list)`.
		//
		//	query:
		...
		//	returns: moor.NodeList
		//		An instance of `moor.NodeList`. Many methods are available on
		...
		//	|	moor.query("#tabular_data > tbody > tr:nth-child(odd)").addClass("odd");
		//	example:
		//		remove all elements with the class "error" from the document
		//		and store them in a list:
		//	|	var errors = moor.query(".error").orphan();
		//	example:
		//		add an onclick handler to every submit button in the document
		//		which causes the form to be sent via Ajax instead:
		//	|	moor.query("input[type='submit']").onclick(function(e){
		//	|		moor.stopEvent(e); // prevent sending the form
		//	|		var btn = e.target;
		//	|		moor.xhrPost({
		//	|			form: btn.form,
		//	|			load: function(data){
		//	|				// replace the form with the response
		//	|				var div = moor.doc.createElement("div");
		//	|				moor.place(div, btn.form, "after");
		//	|				div.innerHTML = data;
		//	|				moor.style(btn.form, "display", "none");
		//	|			}
		//	|		});
		//	|	});
...
var defineAcme= function(){
  ...
  return acme;
	...
	if(this["moor"]){
		...
		defineQuery(this["queryPortability"]||this["acme"]||moor);
	}else{
...
(function(){
	...
	var handlers = _d._contentHandlers = moor.contentHandlers = {
		...
		//	|	moor.contentHandlers.makeCaps = function(xhr){
	...
	moor.__IoArgs = function(){
		...
		//		"moor.preventCache" parameter is sent in the request
		...
		// 		of postData and putData for moor.rawXhrPost and moor.rawXhrPut respectively.
		//	ioPublish: Boolean?
		//		Set this explicitly to false to prevent publishing of topics related to
		// 		IO operations. Otherwise, if djConfig.ioPublish is set to true, topics
		// 		will be published via moor.publish for different phases of an IO operation.
		// 		See moor.__IoPublish for a list of topics that are published.
		//	load: Function?
	...
	/*=====
	moor.__IoCallbackArgs = function(args, xhr, url, query, handleAs, id, canDelete, json){
		//	args: Object
		...
		//		handled.
		//	id: String
		//		For moor.io.script calls only, the internal
		//		script ID used for the request.
		//	canDelete: Boolean
		//		For moor.io.script calls only, indicates
		//		whether the script tag that represents the
		//		request can be deleted after callbacks have
		//		been called. Used internally to know when
		//		cleanup can happen on JSONP-type requests.
		//	json: Object
		//		For moor.io.script calls only: holds the JSON
	...
	/*=====
	moor.__IoPublish = function(){
		// 	summary:
		// 		This is a list of IO topics that can be published
		// 		if djConfig.ioPublish is set to true. IO topics can be
		// 		published for any Input/Output, network operation. So,
		// 		moor.xhr, moor.io.script and moor.io.iframe can all
		// 		trigger these topics to be published.
		//	start: String
		//		"/moor/io/start" is sent when there are no outstanding IO
		// 		requests, and a new IO request is started. No arguments
		// 		are passed with this topic.
		//	send: String
		//		"/moor/io/send" is sent whenever a new IO request is started.
		// 		It passes the moor.Deferred for the request with the topic.
		//	load: String
		//		"/moor/io/load" is sent whenever an IO request has loaded
		// 		successfully. It passes the response and the moor.Deferred
		// 		for the request with the topic.
		//	error: String
		//		"/moor/io/error" is sent whenever an IO request has errored.
		// 		It passes the error and the moor.Deferred
		// 		for the request with the topic.
		//	done: String
		//		"/moor/io/done" is sent whenever an IO request has completed,
		// 		either by loading or by erroring. It passes the error and
		// 		the moor.Deferred for the request with the topic.
		//	stop: String
		//		"/moor/io/stop" is sent when all outstanding IO requests have
		// 		finished. No arguments are passed with this topic.
		this.start = "/moor/io/start";
		this.send = "/moor/io/send";
		this.load = "/moor/io/load";
		this.error = "/moor/io/error";
		this.done = "/moor/io/done";
		this.stop = "/moor/io/stop";
	}
	=====*/
	...
	/*=====
	moor.declare("moor.__XhrArgs", moor.__IoArgs, {
		constructor: function(){
			//	summary:
			//		In addition to the properties listed for the moor._IoArgs type,
			//		the following properties are allowed for moor.xhr* methods.
			//	handleAs: String?
			//		Acceptable values are: text (default), json, json-comment-optional,
			//		json-comment-filtered, javascript, xml. See `moor.contentHandlers`
			//	sync: Boolean?
...
(function(){
	...
	moor.Animation = function(args){
		...
		//	description:
		//		A generic animation class that fires callbacks into its handlers
		//		object at various states. Nearly all moor animation functions
		//		return an instance of this method, usually without calling the
		//		.play() method beforehand. Therefore, you will likely need to
		//		call .play() on instances of `moor.Animation` when one is
		//		returned.
		// args: Object
	...
	/*=====
		// curve: moor._Line|Array
		//		A two element array of start and end values, or a `moor._Line` instance to be
	...
	/*=====
		...
		// beforeBegin: Event?
		//		Synthetic event fired before a moor.Animation begins playing (synchronous)
		beforeBegin: null,

		// onBegin: Event?
		//		Synthetic event fired as a moor.Animation begins playing (useful?)
		onBegin: null,

		// onAnimate: Event?
		//		Synthetic event fired at each interval of a `moor.Animation`
		onAnimate: null,

		// onEnd: Event?
		//		Synthetic event fired after the final frame of a `moor.Animation`
		onEnd: null,

		// onPlay: Event?
		//		Synthetic event fired any time a `moor.Animation` is play()'ed
		onPlay: null,

		// onPause: Event?
		//		Synthetic event fired when a `moor.Animation` is paused
		onPause: null,

		// onStop: Event
		//		Synthetic event fires when a `moor.Animation` is stopped
		onStop: null,
	...
	/*=====
	moor.declare("moor.__AnimArgs", [moor.__FadeArgs], {
		// Properties: Object?
		//	A hash map of style properties to Objects describing the transition,
		//	such as the properties of moor._Line with an additional 'units' property
		properties: {}
	...
	moor.animateProperty = function(/*moor.__AnimArgs*/ args){
		...
		//
		// description:
		//		`moor.animateProperty` is the foundation of most `moor.fx`
		...
		//	|	moor.animateProperty({
		//	|		node: "nodeId",
		//	|		properties: { width: 400 },
		//	|	}).play();
		//		moor figures out the start value for the width and converts the
		...
		//		pedantic way:
		//	|	moor.animateProperty({ node: node, duration:2000,
		//	|		properties: {
		...
		//	|	moor.animateProperty({
		//	|		node: "nodeId",
		//	|		// moor figures out the start value
		...
		//
		// example:
		//		Like all `moor.Animation`s, animateProperty returns a handle to the
		//		Animation instance, which fires the events common to moor FX. Use `moor.connect`
		//		to access these events outside of the Animation definiton:
		//	|	var anim = moor.animateProperty({
		//	|		node:"someId",
		//	|		properties:{
		//	|			width:400, height:500
		//	|		}
		//	|	});
		//	|	moor.connect(anim,"onEnd", function(){
	...
	moor.anim = function(	/*DOMNode|String*/	node,
							...
							/*Integer?*/		delay){
		//	summary:
		//		A simpler interface to `moor.animateProperty()`, also returns
		//		an instance of `moor.Animation` but begins the animation
		//		immediately, unlike nearly every other moor animation API.
		//	description:
		//		`moor.anim` is a simpler (but somewhat less powerful) version
		//		of `moor.animateProperty`.  It uses defaults for many basic properties
		//		and allows for positional parameters to be used in place of the
		//		packed "property bag" which is used for other moor animation
		//		methods.
		//
		//		The `moor.Animation` object returned from `moor.anim` will be
		...
		//		are available in `moor.fx.easing`.
		//	onEnd:
		...
		//	example:
		//		Fade out a node
		//	|	moor.anim("id", { opacity: 0 });
		//	example:
		//		Fade out a node over a full second
		//	|	moor.anim("id", { opacity: 0 }, 1000);
...
moor.provide("moor._base");
	...
	//those browsers, set djConfig.afterOnLoad = true when you know moor is added
	//after page load. Using a timeout so the rest of this
	//script gets evaluated properly. This work needs to happen after the
	//moor.config.require work done in moor._base.
